            ...
            try
            {
                if (OpenConnection() == true)
                {
                    ...
                    command.CommandText = @"Insert INTO imdb.titleprincipals (`tconst`, `ordering`, `nconst`, `category`, `job`, `characters`) 
                                          alues (?tconst, ?ordering, ?nconst, ?category, ?job, ?characters) 
